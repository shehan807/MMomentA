...
CHUNK VERSION: Process a subset of test molecules for parallel execution.

This script:
1. Loads trained MMomentA-GNN model
2. Loads test molecules from HDF5 dataset
3. Processes ONLY molecules in the specified chunk range [start_idx, end_idx)
4. Computes QM ESP for each molecule using Psi4
5. Compares ESP from MPFIT, AM1-BCC, RESP, and MMomentA-GNN charges
6. Saves chunk-specific results with per-atom charges for ALL elements
...
def validate_single_molecule(idx: int, mol_data: MoleculeData, model_dir: Path,
    ...
    try:
        ...
        # Extract atomic numbers for element-wise analysis
        atomic_numbers = mol_data.atomic_features['atomic_numbers']

        # Create element symbols array
        element_symbols = np.array([ATOMIC_SYMBOLS.get(int(z), f'Z{int(z)}') for z in atomic_numbers])
        ...
        return {
            ...
            'mpfit_charges': mpfit_charges.tolist(),  # Convert to list for JSON
            'gnn_charges': gnn_charges.tolist(),
...
def main():
    parser = argparse.ArgumentParser(description="Validate ESP reproduction (CHUNK VERSION)")
    ...
    parser.add_argument("--chunk-id", type=int, required=True,
                       help="Chunk ID (for output filename)")
    parser.add_argument("--start-idx", type=int, required=True,
                       help="Start index in test_indices array")
    parser.add_argument("--end-idx", type=int, required=True,
                       help="End index in test_indices array (exclusive)")
    ...
    parser.add_argument("--n-jobs", type=int, default=8,
                       help="Number of parallel jobs for ESP validation (default: 8)")
    ...
    logger.info("="*60)
    logger.info(f"ESP Validation CHUNK {args.chunk_id}")
    logger.info(f"Indices: [{args.start_idx}, {args.end_idx})")
    logger.info("="*60)
    ...
    logger.info(f"Total test molecules: {len(test_indices)}")
    logger.info(f"Processing chunk {args.chunk_id}: {len(chunk_indices)} molecules")
    ...
    # Store complete results for merging (not summary statistics)
    # This allows the merge script to aggregate across all chunks
    chunk_results = {
        'chunk_id': args.chunk_id,
        'start_idx': args.start_idx,
        'end_idx': args.end_idx,
        'n_molecules': len(chunk_indices),
        'successful': 0,
        'failed': 0,
        'molecule_results': []
    }
    ...
    for result in validation_results:
        if result['success']:
            ...
            chunk_results['molecule_results'].append(result)
        else:
            chunk_results['failed'] += 1
            logger.warning(f"Molecule {result['idx']} failed: {result.get('error', 'Unknown error')}")

    logger.info("="*60)
    logger.info(f"Chunk {args.chunk_id} Complete")
    logger.info(f"Successful: {chunk_results['successful']}/{len(chunk_indices)}")
    logger.info(f"Failed: {chunk_results['failed']}/{len(chunk_indices)}")
    logger.info("="*60)

    # Save chunk results
    results_file = output_dir / f"esp_validation_results_chunk_{args.chunk_id:02d}.json"
    with open(results_file, 'w') as f:
        json.dump(chunk_results, f, indent=2)

    logger.info(f"\n✓ Chunk results saved to {results_file}")
    logger.info("\nDone!")
